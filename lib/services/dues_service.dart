...
class DuesService extends ChangeNotifier {
  ...
  double get totalOutstanding {
    return _duesList
        ...
        .fold(0.0, (sum, item) => sum + item.amountExpected);
  }
  ...
  Future<void> _generateCurrentMonthDueIfMissing() async {
    ...
    try {
      final existing = await _supabase
          ...
          .select()
          .eq('user_id', user.id)
          .eq('due_month', now.month)
          .eq('due_year', now.year);

      if (existing.isEmpty) {
        await _supabase.from('monthly_dues').insert({
          'user_id': user.id,
          'amount_expected': 2000.00, // Default monthly due
          'due_month': now.month,
          'due_year': now.year,
          'status': 'Unpaid',
        });
      ...
      debugPrint('Error checking/generating dues: $e');
    }
  }
  ...
  // Mock payment function to test the DB update
  Future<void> mockPayDue(String dueId) async {
    _isLoading = true;
    notifyListeners();
    try {
      await _supabase
